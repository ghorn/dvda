...
data GNode a = GSource Node a
             | GOutput Node a Node Node -- last node is the output index
...
topSort gnodes = sortBy gCompare gnodes
  where
    gCompare (GOutput _ _ x _) (GOutput _ _ y _) = compare x y
    gCompare (GOutput _ _ _ _) _ = GT
    gCompare _ (GOutput _ _ _ _) = LT
    ...
    parentMap = IntMap.fromList $ foldr f [] gnodes
      where
        ...
        f (GOutput self _ child _) acc = acc ++ [(child, self)]
